...
/obj/structure/machinery/computer/card/ui_static_data(mob/user)
	...
	if(is_centcom)
		...
		departments = list(
			CARDCON_DEPARTMENT_COMMAND = ROLES_CIC,
			CARDCON_DEPARTMENT_AUXCOM = ROLES_AUXIL_SUPPORT,
			CARDCON_DEPARTMENT_MISC = ROLES_MISC,
			CARDCON_DEPARTMENT_SECURITY = ROLES_POLICE,
			CARDCON_DEPARTMENT_ENGINEERING = ROLES_ENGINEERING,
			CARDCON_DEPARTMENT_SUPPLY = ROLES_REQUISITION,
			CARDCON_DEPARTMENT_MEDICAL = ROLES_MEDICAL,

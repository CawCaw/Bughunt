...
#define MODE_NO_SPAWN 			(1<<6) // Disables marines from spawning in normally
#define MODE_XVX				(1<<7) // Affects several castes for XvX, as well as other things (e.g. spawnpool)
#define MODE_NEW_SPAWN 			(1<<8) // Enables the new spawning, only works for Distress currently
#define MODE_DS_LANDED			(1<<9)
#define MODE_BASIC_RT			(1<<10)
#define MODE_RANDOM_HIVE		(1<<11)// Makes Join-as-Xeno choose a hive to join as pooled larva at random rather than at user's input..
...
var/global/list/ROLES_AUXIL_SUPPORT	= list(JOB_PILOT, JOB_CREWMAN)
var/global/list/ROLES_MISC			= list(JOB_SYNTH, JOB_SEA, JOB_CORPORATE_LIAISON, JOB_MESS_SERGEANT)
...
var/global/list/ROLES_WHITELISTED	= list(JOB_SYNTH_SURVIVOR)
var/global/list/ROLES_SPECIAL		= list(JOB_SURVIVOR)

var/global/list/ROLES_REGULAR_ALL 	= ROLES_CIC + ROLES_AUXIL_SUPPORT + ROLES_MISC + ROLES_POLICE + ROLES_ENGINEERING + ROLES_REQUISITION + ROLES_MEDICAL + ROLES_MARINES + ROLES_SPECIAL + ROLES_WHITELISTED + ROLES_XENO
...
#define WHITELISTS_GENERAL				(WHITELIST_COMMANDER|WHITELIST_SYNTHETIC|WHITELIST_MENTOR)
#define WHITELISTS_COUNCIL				(WHITELIST_COMMANDER_COUNCIL|WHITELIST_SYNTHETIC_COUNCIL)
#define WHITELISTS_LEADER				(WHITELIST_COMMANDER_LEADER|WHITELIST_SYNTHETIC_LEADER)
...
#define FACTION_LIST_HUMANOID list(FACTION_MARINE, FACTION_MUTINEER, FACTION_PMC, FACTION_WY, FACTION_DEATHSQUAD, FACTION_CLF, FACTION_UPP, FACTION_FREELANCER, FACTION_SURVIVOR, FACTION_NEUTRAL, FACTION_COLONIST, FACTION_MERCENARY, FACTION_DUTCH, FACTION_HEFA, FACTION_GLADIATOR, FACTION_PIRATE, FACTION_PIZZA, FACTION_SOUTO)
